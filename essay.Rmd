...
```{r rstudio, echo=FALSE, out.extra="", out.width="100%", fig.align="center", fig.cap="RStudio main window. Letter A indicates the Source Pannel; Letter B the Console Pannel; and Letter C the Files/Plots/Packages/Help/Viewer Pannel."}
knitr::include_graphics("figs/RStudio.png")
```

Figure \@ref(fig:rstudio) shows the main RStudio window. To write and edit R code to run your analysis, you need to create a new R script file (File > New File > R Script) that will be shown in the Source Panel, located in the top left part of the screen and indicated by the letter A in the figure. Once ready to run, you can send the code to be evaluated by the software. The code, then, will appear in the Console Panel, along with any output it produces. The Console panel is located below the Source panel, and is indicated by the letter B in the figure. Another important panel is indicated by the letter C. It contains the Plots tab, that will show any graphic produced by your code.

The code can also be typed directly at the Console Panel. It is less advisable, as code typed there cannot be edited or saved to future analysis. To execute code written in a script, put the cursor on the line or highlight the lines (if there are multiple lines to be executed) and press Control + Enter, for Windows, or Command + Enter, for macOS.
...
And we can see that it no longer represents the numeric vector created earlier. Another important thing regarding object names is that is R, the names are case sensitive, hence we need to pay close attention to the spelling.
...
As we can see, typing the name "`My_vector`" with a capital `M` and executing it shows an error message, as an object with this name does not exist. Another important detail is that spaces are not allowed in object names. So to facilitate naming objects, you can follow the tidyverse style guide (accessed through this link: [https://style.tidyverse.org](https://style.tidyverse.org)) and name objects with lower case and separating words with the underscore character "`_`" when needed.
...
We can see what arguments does a function need in their documentation by and putting a question mark before the function name executing it: `?c()`. If you execute a function without specifying a required argument it will raise an error:
...
As can be observed in the Figure \@ref(fig:boxplot) boxplots, pre-test anxiety score values are roughly equal among groups, while the post-test scores tend to be lower for the moderate, but mainly for the high intensity group, compared to the control. This indicates that there is at least a tendency to the exercise to decrease an individual`s anxiety level, and exercise intensity may play a role in it. Also, there is no clear difference in the spread of the scores among groups at both time points.
...
As it showed significant differences between the groups (*p* < 0.001), we can ask for *post hoc* tests to discover between which groups these differences are. We can use the `pairwise.t.test()` function with the Bonferroni adjustment (`p.adjust.method = "bonferroni"`) to run the *post hoc* tests in our non-adjusted means:
...
These results confirm what was shown in Figure \@ref(fig:ancova-plot-no-adj): no significant differences between the control and moderate groups (*p* = 0.281). Now, to represent the ANCOVA results, we will put the adjusted means into a data frame and build the same plot using these means:
...
And we can use the `ggsave()` function of the `ggplot2` package (part of the `tidyverse`) to save our plot into a pdf file in your working directory:
...
The pdf file dimentions can be controlled by the `width` and `height` arguments, and the resolution by the `dpi` argument, according to the journal figure guidelines.
