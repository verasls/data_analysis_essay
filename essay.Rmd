...
R is a programming language and environment for statistical computing and graphics [@R]. It is a free and open source software, which means that the code behind it is fully accessible. R existis as a base package, with some functionality, but one of its greatest advantages is that it can be easily extended by packages, that can be developed and distributed by anyone [@R]. Both R and its packages are stored at the Comprehensive R Archive Network (CRAN) [@R]. Albeit free, powerful and very versatile, R may not be easy to learn. One of the main reasons is that it works as a command line tool, rather than by a graphical user interface, as other statistical softwares such as SPSS. But, once you master a few things, these written commands show to be a more efficient way to work.
...
```{r rstudio, echo=FALSE, fig.pos="H", out.extra="", out.width="100%", fig.align="center", fig.cap="RStudio main window. Letter A indicates the Source Pannel; Letter B the Console Pannel; and Letter C the Files/Plots/Packages/Help/Viewer Pannel."}
knitr::include_graphics("figs/RStudio.png")
```

Figure \@ref(fig:rstudio) shows the main RStudio window. To write and edit R code to run your analysis, you need to create a new R script file (File > New File > R Script) that will be shown in the Source Pannel, located in the top left part of the screen and indicated by the letter A in the figure. Once ready to run, you can send the code to be evaluated by the software. The code, then, will appear in the Console Pannel, along with any output it produces. The Console pannel is located below the Souce pannel, and is indicated by the letter B in the figure. Another important pannel is indicated by the letter C. It contains the Plots tab, that will show any graphic produced by your code.

The code can also be typed directly at the Console Pannel. It is less advisable, as code typed there cannot be edited or saved to future analysis. To execute code written in a script, put the cursor on the line or highlight the lines (if there are multiple lines to be executed) and press Control + Enter, for Windows, or Command + Enter, for macOS.
...
As the assignment operator is used so frequently, RStudio has a shorcut to for it: Alt + minus (-)

#### Every object has a name

Every object, either varible, data, functions, or else, has a name. For example, to the numeric vector we just created was attributed the name "`my_vector`". Each different object you create has to have a different name, otherwise it will overwrite the previous object. For example:
...
Functions are special types of objects that can perform tasks for you. For example, we have already used the function `c()`, which combines elements into a vector. The functions can be recognized by the parenthesis after their names. Insede the parenthesis are the arguments, that are specific information that a function needs to perform its task. Most functions have one or more arguments, and take the general form:
...
We can se what arguments does a function need in their documentation by and putting a question mark before the function name executing it: `?c()`. If tou execute a function without specifying a required argument it will raise an error:
...
The most common class of object to store datasets in R is the data frame. Data frames are a retangular table with rows and columns. We can see an example of a data frame with a tiny dataset contained in the `socviz` package:
...
Another object type that R uses to store datasets and that we will use throughout this tutorial is the tibble. Tibbles are very similar to data frames, but show more infomation regarding the type of data in the columns and are more friendly to interact in the console.
...
An effect size that is commonly calculated in ANCOVA is the partial eta squared ($\eta^2_p$). This effect size corresponds to the proportion of variance that a variable explains and that is not explained by other variables [@Field_2012], and can be expressed as:
