...
By looking at the *p* values (`Pr(>F)` column), we can see that the covariate  significantly predicts the pos-test scores (`pre_test` *p* < 0.001), and that, when removing the effect of the covariate, there is a significant treatment effect (`group` *p* < 0.001). We can, then, look at the descriptive statistics for the post-test scores among groups:
...
According to the ANCOVA results, we can assume that these means are statistically different, although we still cannot say among which groups these differences lies. But, actually, these means have not been adjusted for the covariate, they are the original means. To obtain the adjusted means (also called estimated marginal means or least squares means), we can use the `emmeans()` function:
