...
With this assumption, we want to check if the variance of the outcome variable (post-test scores) is constant across groups. This can be done by executing a Levene's test, which tests the null hypothesis that the variances in different groups are equal. In R, this test can be done by the `leveneTest()` function:
...
As the *p* value (`Pr(>F)` column) is > 0.05, we assume that the null hypothesis is correct and that the variances are equal across the groups.
